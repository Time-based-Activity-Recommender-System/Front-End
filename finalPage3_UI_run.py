...
class FinalPage3(QtGui.QMainWindow):
	...
	reco_books = pd.read_csv('book_reco.data', sep='\t', names=reco_bcols, encoding='latin-1')
        k=0
        for i in range(0,5):
            self.ui.tableWidget_2.setItem(i, 0, QtGui.QTableWidgetItem(reco_books['book_title'][k]))
            k = k + 1
            

	for i in range(0,5):
            self.ui.tableWidget_2.setItem(i, 1, QtGui.QTableWidgetItem(reco_books['book_title'][k]))
            k = k + 1
        ...
        k=0
        for i in range(0,5):
            self.ui.tableWidget_3.setItem(i, 0, QtGui.QTableWidgetItem(reco_movies['movie_title'][k]))
            k = k + 1
